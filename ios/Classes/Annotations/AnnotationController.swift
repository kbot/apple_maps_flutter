...
extension AppleMapController: AnnotationDelegate {
    ...
    private func updateAnnotationOnMap(oldAnnotation: FlutterAnnotation, newAnnotation :FlutterAnnotation) {
        ...
        if let oldAnnotationView = mapView.view(for: oldAnnotation) {
            ...
            oldAnnotationView.layer.zPosition = newAnnotation.zIndex
            ...
            oldAnnotation.zIndex = newAnnotation.zIndex
        }
        else {
            self.removeAnnotation(id: newAnnotation.id)
    ...
    private func addAnnotation(annotation: FlutterAnnotation) {
        if let existingAnnotation = self.getAnnotation(with: annotation.id) {
            updateAnnotationOnMap(oldAnnotation: existingAnnotation, newAnnotation: annotation)
        } else {
